...
if [ $RDBMS == "oracle" ]; then
    ...
    CON_POSTGRES=$(docker $CONTAINER_RUNTIME_ARGS run --tmpfs=/var/lib/postgresql/data:rw,size=1024m -d $CONTAINER_RUN_ARGS -e PGDATA=/pgtmpfs -e POSTGRES_PASSWORD=secret docker.io/postgres:12 -c max_connections=300 -c fsync=off -c synchronous_commit=off -c full_page_writes=off)
    docker $CONTAINER_RUNTIME_ARGS run -d $CONTAINER_RUN_ARGS docker.io/webcenter/activemq:latest
